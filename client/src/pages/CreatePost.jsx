...
const CreatePost = () => {
  ...
  if(form.prompt && form.photo) {
    setLoading(true);
    try{
    const response = await fetch('https://imagefromwords.netlify.app/api/v1/post', {
      method: 'POST',
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://imagefromwords.netlify.app/api/v1/dalle/generate-image', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <section className='max-w-7xl mx-auto'>
      <div>
        ...
        <p className='mt-2 text-[#666e75] text-[16px] max-w-[500px]'>Create imaginative and visually stunning images thorugh DALL-E AI and share them wuith the community</p>
</div>
